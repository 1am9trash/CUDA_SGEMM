...
__global__ void sgemm_thread_register(
    int m, int n, int k,
    ...
    for (int i = 0; i < k; i += BK) {
        ...
        for (int j = 0; j < BK; j++) {
            for (int id_m = 0; id_m < TM; id_m++) {
                frag_a[id_m] = tile_a[(tx + id_m) * BK + j];
            }
            for (int id_n = 0; id_n < TN; id_n++) {
                frag_b[id_n] = tile_b[j * BN + ty + id_n];
            }
            for (int id_m = 0; id_m < TM; id_m++) {
                for (int id_n = 0; id_n < TN; id_n++) {
...
template __global__ void sgemm_thread_register<128, 8, 128, 8, 8>(
    int m, int n, int k,
    const float alpha, const float *a, const float *b, const float beta,
    float *c
);
